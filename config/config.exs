...
config :mauricio,
  ...
  text: %{
    attracted: "Мурмурмур",
    banished: """
    <i><%= cat.name %> даёт дёру от <%= who.full_name %>.</i>
    """,
    start: """
    ...
    away_pet: """
    <i><%= who.full_name %> хочет погладить котяру, но обнаруживает, что того нет дома.</i>
    """,
    awake_pet: """
    <i><%= who.full_name %> гладит котяру.</i>
    """,
    bad_pet: [
      """
      <i><%= cat.name %> недоволен и цапает <%= who.full_name %> за палец.</i>
      ШШШШ.
      """,
      "<i><%= cat.name %> яростно дерется лапой.</i>",
      """
      <i>Когда <%= who.full_name %> пытается погладить котяру, тот прогибает спину и не дает прикоснуться.</i>
      """,
      """
      <i><%= cat.name %> изо всех сил отпинывается от рук <%= who.full_name %> задними лапами.</i>
      """
    ],
    joyful_pet: """
    <i><%= who.full_name %> гладит котика.</i>
    Мурррррррр.
    """,
    sad_pet: "Мямямяууууу...",
    sleep_pet: """
    <i><%= who.full_name %> гладит спящего котика.</i>
    """,
    hug_away: """
    <i><%= who.full_name %> хочет обнять котика, но того нет дома.</i>
    """,
    hug: """
    <i><%= who.full_name %> обнимает <%= state %> котика.
    ...
    aggressive: [
      ...
      "Шшшшшшшшшшшшш....",
      """
      <i><%= cat.name %> утробно рычит на <%= who.full_name %>.</i>
      """,
      "ХСССССССС!!!",
      "У-у-у-рррррр! ШШ!",
    ...
    satiety: %{
      ...
      10 => [
        ...
        """
        <i><%= cat.name %> не притрагивается к еде и смотрит на <%= if not is_atom(who) do who.full_name else "кормушку" end %> тяжелым взглядом, полным бесконечного презрения.</i>
        """,
      ],
      vomit: """
      <i>Котяра съел слишком много, и его стошнило. <%= if not is_atom(who) do who.full_name <> ", это твоя вина!" else "В этом явно виноват только он сам." end %></i>
      """
    },
    stop: "<i>Котяра выпрыгнул с балкона и убежал.</i>",
    food_to_feeder: """
    <i><%= who.full_name %> добавляет</i> <b><%= new_food %></b> <i>в кормушку.</i>
    """,
    feeder_overflow: """
    ...
    fall_asleep: [
      ...
      """
      <i><%= cat.name %> устроился прямо на лучших штанах <%= who.full_name %> и равномерно покрыл их своей шерстью.</i>
      """,
      """
      <i><%= cat.name %> спит на коленях <%= who.full_name %> и пускает слюни.</i>
      """,
      """
    ...
    want_care: [
      """
      <i><%= cat.name %> трется о ногу <%= who.full_name %> и мурчит.</i>
      """,
      """
      <i><%= cat.name %> настойчиво следует за <%= who.full_name %>.</i>
      """,
      "<i><%= cat.name %> громко мяукает в другой комнате.</i>",
      """
      <i><%= cat.name %> принес к ногам <%= who.full_name %> мышь... игрушечную, к счастью.</i>
      """,
      """
      <i><%= cat.name %> кружит вокруг <%= who.full_name %>.</i>
      """,
